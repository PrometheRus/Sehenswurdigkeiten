...
2. Перейти в директорию с плейбуком:
   ...
   ```
3. 4 приватные адреса и 2 публичные адреса ручками добавить в  ```hosts.ini```
   ```
   vi hosts.init
   ```
4. Копируем ранее сгенерированный ssh-ключ и ssh-config на машину-бастион (замените в команде IP _**на публичный IP адрес машины grafana**_) для доступа к виртуальным машинам:
```
