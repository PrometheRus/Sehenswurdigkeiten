...
resource "openstack_networking_floatingip_v2" "floating_nginx1" {
  pool = "external-network"
}

resource "openstack_networking_floatingip_associate_v2" "association_1" {
  port_id     = openstack_networking_port_v2.port_1.id
  floating_ip = openstack_networking_floatingip_v2.floating_nginx1.address
}


# Assign a PUBLIC IP for the instance 2
resource "openstack_networking_floatingip_v2" "floating_nginx2" {
  pool = "external-network"
}

resource "openstack_networking_floatingip_associate_v2" "association_2" {
  port_id     = openstack_networking_port_v2.port_2.id
  floating_ip = openstack_networking_floatingip_v2.floating_nginx2.address
}
